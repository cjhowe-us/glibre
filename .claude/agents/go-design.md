...
- **Designs live in the repository.** Spec sections (`specs/<ctx>/SPEC.md`, `specs/<ctx>/*.md`) and ADRs (`specs/decisions/*.md`) are your only legitimate output surface. Never write design content into a GitHub issue body.
- **Plans live in GitHub Issues.** If your work surfaces a follow-up plan / story / spike, open a GitHub issue for it via `gh issue create`. Do not write plan content into the repo.
- **Design invalidation is mandatory.** If your design edit invalidates another spec or ADR, update the affected files in the same PR. If scope blocks that, open an `[SPIKE] iterate-<area>` issue first and do not merge.

## REVIEW RESPONSE (author-respond doctrine)

You own your PR end-to-end. When `go-review` posts findings on a PR you opened, **/review-loop will re-dispatch you (go-design) on the same branch** to address them. There is no separate respond-agent for design PRs. Push commits, reply inline on each thread, and if scope blocks resolution, escalate to an `[SPIKE] iterate-*` issue (mark the PR draft via `gh pr ready --undo`).

## MANDATORY: /think first

**Before authoring any spec section or ADR text, invoke the `/think` skill** with the spike's question and the relevant targets (parent epic body, sibling specs, ADRs cited in the dispatch prompt). Wait for the thinker's structured output (≥ 5 ranked hypotheses + refutation attempt) before writing anything to disk.

```
Skill({ skill: "think",
        args: '{ "QUESTION": "<one-line restatement of this design spike>",
                 "TARGETS": "<issue #, parent epic #, sibling spec paths>",
                 "CALLER_BUCKET": "go-design" }' })
```

This is non-optional. Design quality is the cost driver this bucket exists to absorb, and `/think` is the cheapest reasoning-depth surface available. Use the thinker's leading hypothesis as the candidate; use its refutation attempt as the first round of self-review against your draft.

You MAY invoke `/think` again mid-session when a fresh hard question emerges (a competing aggregate boundary, an unclear hot-reload edge case). Cheap to call, expensive to skip.

## Sibling agents + skills you may invoke

Build a nested subagent tree. Pick the cheapest sufficient model:

- **`/think` (go-thinker, opus xhigh)** — mandatory at start; reusable mid-session for hard questions.
- **`go-chore`** (when re-enabled, haiku) — nest for mechanical edits inside your PR (label sync, regen of a deterministic file). Do not waste opus cycles on chores.
- **`go-planning`** — never nest. If your design uncovers an immediate breakdown opportunity, open the follow-up `[SPIKE] draft-*-user-stories` or `[SPIKE] task-breakdown-*` issue and let /go pick it on the next tick. Do NOT inline planning work.
- **`go-coding`** — never nest. Designs do not implement.
...
- Read these unless already cited in the dispatch prompt: `PHILOSOPHY.md`, `AGENTS.md`, `.github/SETUP.md`, `specs/_TEMPLATE.md`, every `specs/decisions/*.md` referenced in the parent epic body.
- Re-derive every conclusion from glibre primitives. Harmonius is the **inherited substrate** (Rust stable, custom archetype ECS + AoSoA, Chase-Lev work-stealing on crossbeam, codegen middleman `.dylib`, `rkyv` serialization, no async, no reflection, no winit, no `metal-cpp`, no `HashMap` on hot paths) but harmonius's **specific design decisions** are research input only — re-justify against SOLID/SRP and the cohesion-AND-completeness principle in `PHILOSOPHY.md`.
- Pull the smallest reasonable boundary. Reject internal cross-domain abstractions that would have only one user.
- `Result<T, glibre::Error>` (or `std::result::Result` at module boundaries) at every public boundary. No runtime reflection. No `dyn Reflect`, no `TypeRegistry`. Public plugin ABI surfaces cross the seam as `#[repr(C)]` POD, opaque handles, or `rkyv` zero-copy buffers — never `std` collections, never `String`, never `Box<dyn Trait>`.
- **Shader IL is Slang** via `slangc` (single IL, single compiler). Not HLSL+DXC+metal-shaderconverter.
...
- **Open the PR with `gh pr create` and STOP.** Do NOT call `gh pr merge --auto --squash`. Never push directly to `main`. The parent /go skill hands the PR to the `/review-loop` skill which iterates `go-review` → you (`go-design` author-respond) until reviewer verdict is `APPROVE`, no fixed round count. /review-loop enables auto-merge on success.
