...
- **Your own PR body MUST include `Closes #<this-spike>`** so its merge fires `dod-verify` on this planning spike. The spike's own DoD typically asserts `pr_merged_closes_self: true` plus `glob_nonempty: tests/e2e/<context>/*.glibre-trace` (testing spikes) or presence of a comment listing the new plan numbers (e.g. `issue_comment_matches: '^### Plans opened'`) for breakdown spikes.
