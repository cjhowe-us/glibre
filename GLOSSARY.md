...
- **World** — root container of archetypes, relationships, change ticks.
  Generated by `glibre-codegen` from component schemas (no third-party
  ECS library; custom archetype storage emitted as Rust source).
- **Plugin** — Rust crate compiled into the codegen middleman `.dylib`
  (editor hot reload) or statically linked under LTO (shipping). No
  separate per-plugin `.dylib`.
- **Middleman** — shared `.dylib` carrying all codegen'd component,
  system, material, and blueprint types; recompiled by bundled rustc
  on author-edit; ABI hash gates load.
...
- **Schema** — Rust type layout in the codegen middleman; `rkyv`
  derive emits zero-copy serializers.
- **Type Descriptor** — codegen-emitted Rust constants that drive
  inspector + serialization. No runtime reflection.
...
- **Render Graph** — DAG of passes built each frame in Rust. Never
  serialized; editor visualizes the live graph.
- **Pass** — typed read/write resource declarations + execute
  closure.
- **Resource** — transient (aliased) / persistent / imported texture
  or buffer.
- **Shader Backend** — trait covering compile, reflect, link for
  Slang outputs.
- **Reflection** — `slangc` native reflection record exposing
  bindings + vertex IO.
- **Permutation Key** — `(ShadingModel, Features, RenderPath, LOD)`.
- **PSO** — pipeline state object; cached on
  `(shader_hash, state_hash)`.
- **Meshlet** — ≤64 vert / 124 prim cluster.
- **Cluster DAG** — Nanite-style hierarchical meshlet LOD graph.
- **BLAS / TLAS** — Metal 4 / D3D12 / Vulkan acceleration structures
  for hybrid RT.

## Geometry

- **Mesh** — vertex + index streams.
- **Meshlet Pak** — cooked `rkyv` blob of meshlets + bounds + LOD
  links. Decoded into GPU-resident format at load.

## Tools

- **Scene** — `rkyv` document of entities, components, references;
  plus a deterministic text mirror for diff / merge.
...
- **Plan** — `type:plan`. Leaf implementation unit. Carries a unit
  test plan (≥1 named `#[test]` or integration test). Closed by one
  or more granular Conventional Commit PRs once coding re-enables.
