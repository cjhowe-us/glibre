...
//     3. Captures `context_tag` for future PerfBudget wiring (plan #243
//        will add live record_cpu calls into the S1 fixture).
//
// Usage:
...
//   The test-only executable is outside the engine ABI boundary so this
//   is consistent with policy: engine libraries use -fno-exceptions;
//   test executables may omit the flag when required by test frameworks.
...
//
// Parameters:
//   tag_name    — identifier token; test name becomes "BENCHMARK_CELL_<tag_name>".
//                 Must match the DoD unit_test_named entry exactly.
//   ceiling_ns  — ceiling in nanoseconds (use kCoreCpuBudgetNs etc.).
//   context_tag — glibre::ContextTag value (reserved for plan #243 wiring).
//   catch_tags  — Catch2 tag string tokens, e.g. [perf][core].
//   body_expr   — expression(s) forming the workload; must be evaluatable
//                 as a void expression or return an int-convertible value.
...
#define BENCHMARK_CELL(tag_name, ceiling_ns, context_tag, catch_tags, ...)         \
    TEST_CASE("BENCHMARK_CELL_" #tag_name, #catch_tags) {                          \
        ...
        [[maybe_unused]] constexpr glibre::ContextTag                              \
            GLIBRE_BENCH_CTX_##tag_name = (context_tag);                           \
                                                                                   \
        ...
        const auto GLIBRE_BENCH_NS_##tag_name =                                    \
            glibre::perf_bench::detail::time_body_ns([] {                          \
                __VA_ARGS__;                                                       \
                return 0;                                                          \
            });                                                                    \
        REQUIRE(GLIBRE_BENCH_NS_##tag_name <                                       \
                static_cast<std::uint64_t>(ceiling_ns));                           \
    }
