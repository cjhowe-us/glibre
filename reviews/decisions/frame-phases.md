...
| 7 | render-submit    | `render`       | Record Metal 4 command buffers from `RenderFrame`; mesh-shader gbuffer + hybrid-RT shadow + lighting.  | `RenderFrame` (immutable), GPU resource handles, shader middleman types.       | Metal 4 command buffers, transient GPU resources; signals submit-fence.                    | Command buffer for frame N is enqueued to the GPU; sim-side reads of `RenderFrame` are done.                 |
