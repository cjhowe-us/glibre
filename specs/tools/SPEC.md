...
true for **editor builds** and the **E2E test runner** and false for
the runtime / shipping build (PHILOSOPHY §6 — no runtime reflection in
shipping builds). The codegen column descriptors the overlay reads
live in the middleman dylib (`glibre-types.dylib`) regardless of
build flavour, so toggling the feature flag changes only whether the
overlay's *handlers* link in — not whether the *types* are reachable.
A shipping runtime that links `glibre-types.dylib` cannot reach
`DebugOverlay`'s handlers because the symbols are stripped at link
...
`ReflectionBlob` reader (§4.4), and the toolbar's mode-transition
controls (§4.8) do not.

This aggregate is the resolution of spike #863 (this section). The
component-mutation E2E trace at `tests/e2e/core/component-mutation.glibre-trace`
is the first artefact to cite this anchor; future traces follow the
same citation pattern (see invariant 6 below).

**Composition.** Three named parts:
...
- **`OverlayCaptureTable`** — a per-`World` typed map keyed by a
  ...
  table lives in `EditorWorld` even when the path's prefix names a
  `GameWorld` (e.g. `world.A.…`); §4.10 inv. 1 (worlds disjoint) is
  preserved because the table's entries are *snapshots of*
  `GameWorld` state, not borrowed references into it.
- **`Binding`** — a value-object pairing a `platform::Key` (always
  one of `F1..F7` in MVP), a typed action descriptor (a closed-sum
...
1. **Auto-publish before handler return.** Every `DebugOverlay`
   ...
   `TraceRecorder` snapshot taken between two handler invocations
   sees either the *complete* pre-press snapshot or the *complete*
   post-press snapshot, never a partial / torn intermediate. This
   is the load-bearing invariant the `.glibre-trace` block-3
   correctness argument depends on (see invariant 6).
2. **Per-binding atomicity.** The unit of atomicity is **one
   binding press**. All records a single binding declares are
   published as one atomic step; records from different bindings
   are serialised by press order within the input phase. Frame-
   ordering across bindings is carried by the input phase's
...
3. **Frame-boundary completeness.** No `OverlayCapture` write
   ...
   `core::FrameLoop` phase boundaries (`reviews/decisions/frame-phases.md`
   phases 1–9) are the timing reference; this aggregate registers
   no phase of its own (§4.10 inv. 10).
4. **Action handlers run in phase 1 (input), against `GameWorld`
   under tools' write window.** A binding press becomes a
   `platform::InputEvent::KeyDown` in phase 1; the
   `DebugOverlay` action handler is registered as a system inside
   phase 1 (provisional placement per `frame-phases.md` open
   question 3 — tools writes happen in phase 1 before any sim
   consumer reads). The handler is the only path through which
   the overlay mutates `GameWorld`; it does not bypass `core`'s
   public `World::set_component` / `World::remove_component` ABI
   (`specs/core/SPEC.md` §5.5). Game-world mutations issued by
   debug-overlay actions therefore obey the same archetype-
...
5. **Non-perturbing capture.** The act of capturing pre / post
   ...
   subscription / unsubscription must be balanced and may not
   leave residual subscribers after the handler returns. This
   mirrors §4.9 inv. 1 (the recorder's non-perturbing-capture
   rule) — debug-overlay observation is the same kind of side
...
6. **Stable spec citation anchor.** This section (§4.11) is the
   ...
   binding set and the per-binding capture list (existing
   bindings keep their press semantics + capture paths byte-equal
   across edits); a non-additive change requires a new spec
   anchor (`§4.11.v2`) so old traces continue to cite the
   contract they were authored against.
7. **No shipping reach.** The `DebugOverlay` resource and its
   handlers do not link into shipping runtime builds.
...
key order (`F8` next) and become §4.11 entries; per invariant 6, the
existing seven entries remain stable so old traces keep citing the
contract they were authored against.
...
- **§4.9 `TraceRecorder`** — `TraceRecorder` writes
...
- **§4.8 `Toolbar`** — the toolbar owns `EditorMode` transitions
  via `PlayPauseStep`; debug-overlay bindings are independent of
  `EditorMode` (the overlay fires in `Edit`, `Paused`, `Step`,
  `Play`, and `Recording` modes alike, since debug actions are
...
- **§4.10 cross-aggregate invariants 1, 3, 5, 9, 10** — preserved:
  ...
  edits** — they are debug-only `GameWorld` mutations gated
  behind a non-shipping feature flag, and they go through
  `core::World`'s public `set_component` / `remove_component`
  ABI rather than constructing `EditCommand` envelopes (the
  envelope shape is reserved for user-authored, undoable
  edits per §4.7); non-perturbing capture is preserved per
  invariant 5; hot-reload survival rides the standard middleman
  schema rule; tools registers no new phase, the action handler
  runs inside phase 1.

**Failure modes.** Debug-overlay handlers return
`std::expected<void, glibre::Error>` per `reviews/decisions/error-model.md`.
The expected closed sum of arms a handler may surface:
