...
// Source: data/schemas/<ctx>/<Type>.fory  (sha256 of canonical
...
4. Compute `blake3(concatenation)` — the resulting 32-byte digest
...
extern "C" const char* glibre_plugin_abi_hash() noexcept;
// Body lives in glibre-types.dylib via the ::glibre_types_abi_hash
// re-export emitted in §3.6.3.
```
...
enum class DiagnosticKind : std::uint16_t {
    ...
    TypeGraphCycle        = 7,  // → MigrationCycle (codegen-time, type-graph variant)
...
Source list, in canonical order (SPEC §4.4 inv. 1, §6.4):

1. Every `data/schemas/**/*.fory` source's blake3 over its
   canonical form (§7.3 of SPEC; §3.3 stage 3 of this design),
   including `data/schemas/meta/*.fory` and
   `data/schemas/core/PluginManifest.fory`.
2. For each schema, the declared `version` integer (4 bytes,
   little-endian).
3. For each schema, the `fqn` (UTF-8 bytes).

Concatenation order is canonical Unicode code-point order over
`fqn`. The 32-byte digest is hex-encoded (lowercase, 64 chars)
and embedded by `_abi_hash.cpp` (§3.6).
