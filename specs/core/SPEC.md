...
| `TypeRegistryGap`                | `register_type()` or `extend_during_load()` called with a `TypeId.value` that exceeds the current dense storage end by more than 1, which would create a gap in the `TypeId` sequence. Caller must register intermediate IDs first or compact the registry (§4.9 #3). | Refuse     | none                                         | `error`  |
